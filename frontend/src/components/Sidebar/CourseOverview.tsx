...
const CourseOverview = memo(function CourseOverview({
  ...
  // Formatted forecast horizon date from API (or null if unavailable)
  const forecastHorizonDate = useMemo(() => {
    if (!raceForecast?.forecast_horizon) return null;
    return formatDate(raceForecast.forecast_horizon);
  }, [raceForecast]);

  // "Check back in ~N days" message based on gap between horizon and earliest checkpoint
  const checkBackMsg = useMemo(() => {
    if (!raceForecast?.forecast_horizon || raceForecast.checkpoints.length === 0) {
      return formatCheckBackMessage(null, "");
    }
    const earliestTime = raceForecast.checkpoints.reduce(
      (min, cp) => (cp.expected_time < min ? cp.expected_time : min),
      raceForecast.checkpoints[0]!.expected_time,
    );
    return formatCheckBackMessage(raceForecast.forecast_horizon, earliestTime);
  }, [raceForecast]);
        ...
        {allUnavailable ? (
          <div className="mt-4 rounded-lg bg-surface-alt p-4">
            <p className="text-sm text-text-muted">
              ...
              current forecast horizon{forecastHorizonDate && (
                <> (<span className="text-text-secondary">{forecastHorizonDate}</span>)</>
              )}.
              {" "}{checkBackMsg}
            </p>
          </div>
